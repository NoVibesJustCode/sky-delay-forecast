...
3.  **OpenWeatherMap Feeder**:
    Requires the broker URL, the topic names (weather and forecast) and an external file containing airport information (IATA codes, locations, etc.). You can find a template for the required format in ```storage/references/airports.csv```.
...
4.  **Flight Status Feeder**:
    Requires the broker URL, the topic name (flight) and the path to the file that stores the flight links generated by the crawler for later scraping (e.g., ```storage/data/pending_flight_links.txt```).
    ```bash
