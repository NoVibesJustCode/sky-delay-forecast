...
1.  **Start the Broker**: Ensure ActiveMQ is active (port 61616 by default).

2.  **Event Store Builder**:
    Requires the broker URL.
    ```bash
    tcp://localhost:61616
    ```

3.  **OpenWeatherMap Feeder**:
    Requires the broker URL, the topic names and the airports data file.

    **Args**:
    ```bash
    tcp://localhost:61616 weather forecast storage/references/airports.csv 
    ```
...
4.  **Flight Status Feeder**:
    Requires the broker URL, the topic name and the path to the file that stores the flight links generated by the crawler for later scraping.
    ```bash
    tcp://localhost:61616 flight storage/logs/pending_flight_links.txt
    ```

5.  **Business Unit**:
    Requires the broker URL, the Event Store path, the SQLite database path and the airport data file.
    ```bash
    tcp://localhost:61616 eventstore/ storage/db/datamart.db storage/references/airports.csv 
    ```
